...
int main() {
    int values[] = {5000000, 10000000, 15000000, 20000000, 25000000};
    printf("n,quickSort,radixSort\n");
    ...
    for (i = 0; i < 5; i++) {
        ...
        int* array = malloc(sizeof(int) * values[i]);
        int j = 0;
        ...
        int* copy = copyArray(array, values[i]);
        start = clock();
        quickSort(copy, 0, values[i] - 1);
        end = clock();
        printf("%.2f,", (end - start) / (double)(CLOCKS_PER_SEC));
        free(copy);

        copy = copyArray(array, values[i]);
        start = clock();
        radixSort(copy, values[i], 2);
        end = clock();
