...
void mergeSortR(int* a, int start, int end, int* b, int* counter) {
    if (start >= end)
        return;

    int middle = start + (end - start) / 2;
    mergeSortR(a, start, middle, b, counter);
    mergeSortR(a, middle + 1, end, b, counter);
    *counter += merge(a, start, middle, end, b);
}
...
long radixSort(int *a, int n, int p) {
    ...
    for (k = 0; k < p; k++) {
        ...
        for (i = 0; i < n; i++) {
            int val = offset[(a[i] / (int) pow(10, k)) % 10]++;
            tmp[val] = a[i];
